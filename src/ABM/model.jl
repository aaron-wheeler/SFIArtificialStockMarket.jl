...
function model_step!(model)
    ...
    for agent in scheduled_agents
        ...
        if in.(model.t, Ref(agent.δ)) == true
            ...
            if rand() ≤ model.pGAcrossover
                for i in 1:model.num_elimination
            ...
            df_GA[100, :predictors][1] = sum(df_GA[i, :predictors][1]*(1/df_GA[i, :predictors][3]) 
                for i in 1:(num_predictors-1)) / sum(1/df_GA[i, :predictors][3] for i in 1:(num_predictors-1)) # default a
            df_GA[100, :predictors][2] = sum(df_GA[i, :predictors][2]*(1/df_GA[i, :predictors][3])
                for i in 1:(num_predictors-1)) / sum(1/df_GA[i, :predictors][3] for i in 1:(num_predictors-1)) # default b
    ...
    for agent in scheduled_agents
        ...
        if model.t > model.generalization_t
            for i in 1:model.num_predictors
