...
- Among the active predictors, select the one with the best current prediction accuracy. If multiple:
...
function match_predictors(id, num_predictors, predictors, state_vector, predict_acc, fitness_j, σ_pd)
    ...
    # Set chosen_j to index of predictor to be used for agent demand
    chosen_j = 0
...
function update_predict_acc!(agent, τ, price, dividend)
    for i = 1:length(agent.predict_acc)
        if i .∈ Ref(agent.active_predictors)
            ...
            # if agent.predict_acc[i] > 50.0
            #     agent.predict_acc[i] = 50.0
            # end
        end
    end
end
